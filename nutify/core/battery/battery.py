    ...
    try:
        ...
        if not has_valid_data:
            try:
                ...
                latest_data = UPSDynamicData.query.order_by(UPSDynamicData.timestamp_tz.desc()).first()
                ...
                if events:
                    ...
                    for event in events:
                        if event.event_type == 'ONBATT':
                            ...
                            duration = (event.timestamp_tz - open_event.timestamp_tz).total_seconds()
                            paired_events.append({
                                'start': open_event.timestamp_tz,
                                'end': event.timestamp_tz,
                                'duration': duration
                            })
                    ...
                    if open_event:
                        duration = (end_time - open_event.timestamp_tz).total_seconds()
                        paired_events.append({
                            'start': open_event.timestamp_tz,
                            'end': end_time,
                            'duration': duration
...
def _create_default_battery_stats():
    """Create default battery statistics when no data is available"""
    return {
        'battery_charge': {
            'min': 100, 
            'max': 100, 
...
def get_battery_history(period='day', from_date=None, to_date=None, selected_date=None, today_mode=None):
    ...
    try:
        ...
        for metric in metrics:
            try:
                if hasattr(UPSDynamicData, metric):
                    # Query for data points in the given time range
                    ...
                    if data_points:
                        ...
                        first_timestamp = data_points[0].timestamp_tz
                        last_timestamp = data_points[-1].timestamp_tz
                        ...
                        for point in data_points:
                            ...
                            if value is not None:
                                # Don't convert runtime from seconds to minutes anymore
                                # Keep all values in original units
                                value = float(value)
                                
                                history[metric].append({
                                    'timestamp': point.timestamp_tz.isoformat(),
                                    'value': value
                                })
                        ...
                        # Check if we have too many points after sampling and limit if needed
                        if len(history[metric]) > target_points:
                            history[metric] = history[metric][:target_points]
                else:
        ...
        try:
            history['events'] = [{
                ...
                'end_time': event.timestamp_tz_end.isoformat() if hasattr(event, 'timestamp_tz_end') and event.timestamp_tz_end else event.timestamp_tz.isoformat()
            } for event in events]
        except Exception as e:
