...
from core.db.ups.utils import get_configured_timezone, ups_config
...
def create_static_model(db=None):
    ...
    class UPSStaticData(db.Model):
        ...
        def __repr__(self):
            return f"<UPSStaticData {self.id} - {self.timestamp_tz}>"
...
def create_dynamic_model(db=None):
    ...
    class UPSDynamicData(db.Model):
        ...
        def __repr__(self):
            return f"<UPSDynamicData {self.id} - {self.timestamp_tz}>"
...
def initialize_static_data(db):
    ...
    try:
        ...
        static_fields = [c.name for c in UPSStaticData.__table__.columns 
                        if c.name not in ('id', 'timestamp_tz')]
...
def initialize_static_data_if_needed(db):
    ...
    try:
            ...
            if schema_mismatch:
                ...
                try:
                    ...
                    with db.engine.connect() as conn:
                        conn.execute(text("DROP TABLE IF EXISTS ups_static_data"))
                        conn.commit()
        ...
        try:
            ...
            result = conn.execute(text("SELECT COUNT(*) FROM ups_static_data")).scalar()
            conn.close()
...
def insert_initial_dynamic_data(db):
    ...
    try:
        ...
        try:
            # Use raw SQL to avoid transaction issues
            conn = db.engine.connect()
            result = conn.execute(text("SELECT COUNT(*) FROM ups_dynamic_data")).scalar()
            conn.close()
