            ...
            # If ups_realpower_hrs is not available, try to calculate from load and nominal power
                ...
                if len(valid_data_points) == 1:
                    ...
                    for i in range(1, len(valid_data_points)):
                        ...
                        time_diff = (valid_data_points[i].timestamp_tz - valid_data_points[i-1].timestamp_tz).total_seconds() / 3600
...
def format_cost_series(data, energy_type='realtime'):
    ...
    try:
        ...
        if energy_type == 'realtime':
            for row in data:
                ...
                if hasattr(row, 'ups_realpower') and row.ups_realpower is not None:
                    ...
                    series.append({
                        'x': row.timestamp_tz.timestamp() * 1000,  # Convert to milliseconds
        ...
        elif energy_type == 'days':
            for row in data:
                if hasattr(row, 'ups_realpower_days') and row.ups_realpower_days is not None:
                    energy_kwh = float(row.ups_realpower_days) / 1000  # Convert Wh to kWh
                    cost = energy_kwh * rate
                    series.append({
                        'x': row.timestamp_tz.timestamp() * 1000,  # Convert to milliseconds
                        'y': round(cost, 2)
            ...
            # Calculate based on ups_load and ups_realpower_nominal
            prev_timestamp = None
            prev_power = None
            ...
            for row in data:
                ...
                current_timestamp = row.timestamp_tz
                current_power = (float(row.ups_realpower_nominal) * float(row.ups_load)) / 100  # W
                
                if prev_timestamp is not None and prev_power is not None:
                    # Calculate duration in hours
                    duration = (current_timestamp - prev_timestamp).total_seconds() / 3600
                    
                    # Skip if time diff is too large
                    if duration > 2:
                    ...
                    series.append({
                        'x': current_timestamp.timestamp() * 1000,  # Convert to milliseconds
                        'y': round(cost, 4)
                    })
            ...
            if len(valid_data_points) == 1:
                ...
                for row in valid_data_points:                    
                    ...
                    if prev_timestamp is not None and prev_power is not None:
                        ...
                        mid_timestamp = prev_timestamp + (current_timestamp - prev_timestamp) / 2
                        hour = mid_timestamp.hour
    ...
    try:
        ...
        if has_hrs_data:
            ...
            for row in data:
                ...
                day_key = row.timestamp_tz.strftime('%Y-%m-%d')
            ...
            for day_key, records in day_records.items():
                ...
                records.sort(key=lambda x: x.timestamp_tz)
                ...
                if len(records) > 1:
                    ...
                    for row in records:
                        ...
                        current_power = (float(row.ups_realpower_nominal) * float(row.ups_load)) / 100
