...
def verify_ups_identity():
    ...
    device_mfr, device_model, device_serial
    """
    try:
        # Get the current UPS data
        current_ups = get_available_variables()
        
        # Check the presence of the three required fields
        required_fields = ['device.mfr', 'device.model', 'device.serial']
        for field in required_fields:
            if field not in current_ups:
                msg = f"Required field {field} not found in UPS data"
                logger.error(msg)
                return False, msg
        ...
        try:
            # Direct query to read only the necessary fields
            result = db.session.execute(text("""
                SELECT device_mfr, device_model, device_serial 
            ...
            stored_values = {
                'device_mfr': result[0] if result[0] else '',
                'device_model': result[1] if result[1] else '',
                'device_serial': result[2] if result[2] else ''
            }
        ...
        # Compare the values
        current_values = {
            'device_mfr': current_ups['device.mfr'].strip(),
            'device_model': current_ups['device.model'].strip(),
            'device_serial': current_ups['device.serial'].strip()
        }
        
        # If even one value is different, the UPS is different
        if (current_values['device_mfr'] != stored_values['device_mfr'] or
            current_values['device_model'] != stored_values['device_model'] or
            current_values['device_serial'] != stored_values['device_serial']):
            
            msg = (
                f"Different UPS detected!\n"
                f"Current: Mfr={current_values['device_mfr']}, "
                f"Model={current_values['device_model']}, "
                f"Serial={current_values['device_serial']}\n"
                f"Stored: Mfr={stored_values['device_mfr']}, "
                f"Model={stored_values['device_model']}, "
                f"Serial={stored_values['device_serial']}"
            )
            logger.warning(msg)
            return False, msg
