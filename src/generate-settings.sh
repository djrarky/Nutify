...
show_motd() {
  ...
  echo ""
  echo "  _   _ _   _ _____ ___ _______   __"
  echo " | \ | | | | |_   _|_ _|  ___\ \ / /"
  echo " |  \| | | | | | |  | || |_   \ V / "
  echo " | |\  | |_| | | |  | ||  _|   | |  "
  echo " |_| \_|\___/  |_| |___|_|     |_|  "
  echo ""
  echo "  Network UPS Tools Interface v${VERSION}"
  echo "  Last Update: ${LAST_UPDATE}"
  echo "  https://github.com/DartSteven/nutify"
  echo ""
...
# Only redirect output after MOTD and timezone setup
if [ "$ENABLE_LOG_STARTUP" != "Y" ]; then
  exec > /dev/null 2>&1
fi
...
UPSCMD_USER = ${UPSCMD_USER}
UPSCMD_PASSWORD = ${UPSCMD_PASSWORD}
...
MONITOR ${UPS_NAME:-ups}@localhost 1 ${UPS_USER:-admin} ${UPS_PASSWORD:-hunter2} master
MINSUPPLIES 1
...
for file in $essential_file1 $essential_file2 $essential_file3 $essential_file4 $essential_file5; do
  if [ ! -f "/etc/nut/$file" ]; then
    startup_log "ERROR: Essential file /etc/nut/$file is missing"
    missing=$((missing + 1))
  fi
done
